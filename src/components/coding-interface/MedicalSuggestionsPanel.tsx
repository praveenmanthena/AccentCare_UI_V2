...
import { Search, Plus, MessageSquare, Calculator, TrendingUp, Award, Star, Shield, ChevronDown, ChevronUp, Info, X, AlertTriangle, ArrowDown, ArrowUp, Save, CheckCircle } from 'lucide-react';
...
import { Comment, CodeSuggestion as CodeSuggestionType } from '../../types';
...
export const MedicalSuggestionsPanel: React.FC<MedicalSuggestionsPanelProps> = ({
  codingState,
  icdSearch,
  getFilteredCodes,
  commentEditMode,
  ...
  const [draggedSuggestion, setDraggedSuggestion] = useState<CodeSuggestionType | null>(null);
  ...
  const pendingCode = codingState.secondarySuggestions.find((code: any) => code.id === codingState.pendingPromotionCodeId);

  // Helper function to find suggestion by ID
  const findSuggestion = (id: string) => {
    const primary = codingState.primarySuggestions.find((s: any) => s.id === id);
    if (primary) return { suggestion: primary, container: 'primary' };
    
    const secondary = codingState.secondarySuggestions.find((s: any) => s.id === id);
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    const activeContainer = codingState.primarySuggestions.find((s: any) => s.id === activeId) ? 'primary' : 'secondary';
    const overContainer = codingState.primarySuggestions.find((s: any) => s.id === overId) ? 'primary' : 'secondary';
    ...
    if (activeContainer === overContainer) {
      ...
      if (activeContainer === 'primary') {
        codingState.setPrimarySuggestions((prev: any) => {
          ...
          return reordered.map((code: any, index: number) => ({ ...code, order: index }));
        });
      } else {
      ...
      const sourceList = activeContainer === 'primary' ? codingState.primarySuggestions : codingState.secondarySuggestions;
      const targetList = overContainer === 'primary' ? codingState.primarySuggestions : codingState.secondarySuggestions;
      const setSourceList = activeContainer === 'primary' ? codingState.setPrimarySuggestions : codingState.setSecondarySuggestions;
      const setTargetList = overContainer === 'primary' ? codingState.setPrimarySuggestions : codingState.setSecondarySuggestions;
      ...
      // Remove from source
      const newSourceList = sourceList.filter((item: any) => item.id !== activeId);
      ...
      setSourceList(newSourceList.map((code: any, index: number) => ({ ...code, order: index })));
      setTargetList(newTargetList.map((code: any, index: number) => ({ ...code, order: index })));
    }
  ...
  const canComplete = codingState.pendingDecisions === 0 && codingState.hasExactlyOneActivePrimaryCode;
  ...
  return (
    ...
    >
      ...
      <div className="border-b border-gray-200 p-4">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <h2 className="text-lg font-bold text-gray-800">Medical Suggestions</h2>
                ...
                <div className="absolute top-full left-0 mt-2 w-80 bg-white border border-gray-200 rounded-lg shadow-lg z-50 p-4">
                  ...
                  <div className="space-y-3 text-sm">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-gray-800 mb-1">Reordering</p>
                        <p className="text-gray-600 leading-relaxed">
                          Drag codes to reorder them within Primary or Secondary sections.
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-gray-800 mb-1">Cross-Section Movement</p>
                        <p className="text-gray-600 leading-relaxed">
                          Drag codes between <strong>Primary</strong> and <strong>Secondary</strong> sections to change their classification.
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="pt-2 border-t border-gray-100">
                      <p className="text-xs text-gray-500 font-medium">
                        💡 <strong>Tip:</strong> Use the grip handle (⋮⋮) to drag codes around.
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <div className="text-left">
                  <div className="text-sm font-bold text-blue-800">HIPPS Details</div>
                  <div className="text-xs text-blue-600">
                    {codingState.hippsScore} • ${codingState.finalPayment.toLocaleString()}
                  </div>
                </div>
              </div>
            </button>
            ...
            >
              <Save className={`w-4 h-4 ${codingState.isSaving ? 'animate-spin' : ''}`} />
        ...
        <div className="mb-4">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-xs font-medium text-gray-500">Decision Progress</h3>
            <div className="flex items-center gap-3">
              <span className="text-xs text-gray-500">
                {codingState.completedDecisions} of {codingState.aiSuggestionsCount} recommendations completed
              </span>
        ...
        <div className="flex gap-1 bg-gray-100 p-1 rounded-lg">
          ...
          >
            ...
            <span className="ml-1 px-2 py-0.5 bg-gray-200 text-gray-700 rounded-full text-xs font-bold">
              {codingState.primarySuggestions.filter((code: any) => !codingState.rejectedCodes.has(code.id)).length + 
               codingState.secondarySuggestions.filter((code: any) => !codingState.rejectedCodes.has(code.id)).length}
            </span>
          </button>
          <button
      ...
      <div className="flex-1 overflow-auto p-4">
        ...
        {codingState.activeTab === 'all' && codingState.searchTerm.length < 2 ? (
          <>
            ...
            <div className="mb-6 bg-green-50 border border-green-200 rounded-lg p-4">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-lg font-bold text-green-800">
                    Primary ICD Suggestions ({codingState.primarySuggestions.filter((code: any) => !codingState.rejectedCodes.has(code.id)).length})
                  </h3>
                </div>
                <button 
                  onClick={() => icdSearch.startAddingICD('primary', codingState.hasActivePrimaryCode)}
                  ...
                  disabled={icdSearch.isAddingICD || codingState.hasActivePrimaryCode}
                  title={codingState.hasActivePrimaryCode ? 'Cannot add multiple primary codes. Demote or reject the current primary code first.' : 'Add new primary ICD code'}
                >
              ...
              <SortableContext 
                items={codingState.primarySuggestions
                  .filter((code: any) => !codingState.rejectedCodes.has(code.id))
              ...
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-lg font-bold text-blue-800">
                    Secondary ICD Suggestions ({codingState.secondarySuggestions.filter((code: any) => !codingState.rejectedCodes.has(code.id)).length})
                  </h3>
                </div>
                <button 
                  ...
                  className={`px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm font-bold transition-all duration-200 flex items-center gap-2 ${
                    icdSearch.isAddingICD ? 'opacity-50 scale-95' : 'opacity-100 scale-100'
                  }`}
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold text-gray-800 capitalize">
                {codingState.activeTab === 'newly-added' ? 'Newly Added' : codingState.activeTab} Codes
              </h3>
            ...
            {getFilteredCodes().length > 0 ? (
              getFilteredCodes().map((suggestion, index) => (
                <CodeSuggestion 
                  ...
                  suggestion={suggestion} 
                  index={index} 
                  isPrimary={codingState.primarySuggestions.some((s: any) => s.id === suggestion.id)}
                  showUndo={true}
                  undoAction={
                    codingState.activeTab === 'accepted' 
        ...
        {activeId && draggedSuggestion ? (
          <div className="opacity-60">
            <CodeSuggestion
              ...
              index={0}
              isPrimary={codingState.primarySuggestions.some((s: any) => s.id === activeId)}
              showUndo={false}
              undoAction={null}
              showMoveActions={false}
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full m-4">
            <div className="flex items-center justify-between p-6 border-b border-gray-200">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-lg font-bold text-gray-800">Cannot Add Primary Code</h2>
              </div>
              <button
            ...
            <div className="p-6 space-y-4">
              <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
                <p className="text-sm text-orange-800 font-medium mb-3">
                  You already have an active primary diagnosis code. Only one primary diagnosis is allowed per episode.
                </p>
                
                {activePrimaryCode && (
                  <div className="bg-white border border-orange-200 rounded-lg p-3">
                    <h4 className="text-sm font-bold text-gray-800 mb-2">Current Primary Diagnosis:</h4>
                    <div className="flex items-center gap-3">
              ...
              <div className="space-y-3">
                ...
                <ul className="space-y-2 text-sm text-gray-600">
                  <li className="flex items-center gap-2">
                    ...
                    <span><strong>Demote</strong> the current primary to secondary, or</span>
                  </li>
                  <li className="flex items-center gap-2">
                    <div className="w-2 h-2 bg-red-500 rounded-full"></div>
                    <span><strong>Reject</strong> the current primary diagnosis</span>
                  </li>
                </ul>
                
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                  <p className="text-xs text-blue-700 font-medium">
                    💡 <strong>Tip:</strong> You can use the arrow buttons on the current primary code to demote it to secondary, or use the reject button to remove it entirely.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-lg w-full m-4">
            <div className="flex items-center justify-between p-6 border-b border-gray-200">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-lg font-bold text-gray-800">Primary Code Conflict</h2>
              </div>
              <button
            ...
            <div className="p-6 space-y-4">
              <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
                ...
                {pendingCode && (
                  <div className="mb-4">
                    <h4 className="text-sm font-bold text-gray-800 mb-2">Code to Promote:</h4>
                    <div className="bg-white border border-orange-200 rounded-lg p-3">
                      <div className="flex items-center gap-3">
              ...
              <div className="space-y-3">
                <p className="text-sm font-medium text-gray-700">
                  To proceed, you need to handle the current primary diagnosis first:
                </p>
                ...
                <div className="space-y-2">
                  {pendingCode && (
                    ...
                    >
                      ...
                      <div>
                        <div className="font-semibold text-blue-800">Demote to Secondary</div>
                        <div className="text-sm text-blue-600">
                          Move current primary to secondary and promote the selected code
                        </div>
                      </div>
                    </button>
                  )}
                  ...
                  <button
                    onClick={pendingCode ? codingState.handleRejectCurrentPrimary : codingState.handleAcceptPendingPrimary}
                  ...
                  >
                    ...
                    <div>
                      <div className="font-semibold text-red-800">Reject Current Primary</div>
