...
import { 
  Check, X, Award, MessageSquare, ChevronDown, ChevronUp, 
  Plus, RotateCcw, ExternalLink, Move, GripVertical
} from 'lucide-react';
...
export const CodeSuggestion: React.FC<CodeSuggestionProps> = ({
  ...
  const hippsStyle = suggestion.isHippsContributor ? getHippsPriorityStyle(suggestion.hippsPoints) : null;
    ...
    >
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          <span className={`px-3 py-2 rounded-full text-sm font-bold flex-shrink-0 transition-all duration-200 ${
            isPrimary 
          ...
          {suggestion.isHippsContributor && hippsStyle && (
            <div className={`flex items-center gap-1 flex-shrink-0 px-2 py-1 rounded border transition-all duration-200 ${hippsStyle.bgColor} ${hippsStyle.textColor} ${hippsStyle.borderColor}`}>
      ...
      {suggestion.isHippsContributor && suggestion.hippsPoints >= 20 && (
        <div className="mt-2 p-2 bg-red-50 border border-red-200 rounded-lg transition-all duration-200">
          ...
          <p className="text-xs text-red-700 mt-1 font-medium">
            This code significantly impacts reimbursement. Review carefully before making decisions.
          </p>
        </div>
      )}
      ...
      {isExpanded && (
        <div className="mt-3 pt-3 border-t border-gray-200 space-y-4">
          {/* Description */}
          <div>
            <p className="text-gray-800 font-semibold text-sm">{suggestion.description}</p>
          </div>

          {/* HIPPS Information - Detailed view when expanded */}
          {suggestion.isHippsContributor && (
            <div className={`p-3 rounded-lg border transition-all duration-200 ${hippsStyle?.bgColor} ${hippsStyle?.borderColor}`}>
              <h4 className={`font-bold mb-2 text-sm flex items-center gap-2 ${hippsStyle?.textColor}`}>
                <Award className="w-4 h-4" />
                HIPPS Score Impact
              </h4>
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <span className={`font-semibold ${hippsStyle?.textColor}`}>Points:</span>
                  <span className={`ml-2 font-bold ${hippsStyle?.textColor}`}>+{suggestion.hippsPoints}</span>
                </div>
                <div>
                  <span className={`font-semibold ${hippsStyle?.textColor}`}>Priority:</span>
                  <span className={`ml-2 font-bold ${hippsStyle?.textColor}`}>{hippsStyle?.priority}</span>
                </div>
                <div>
                  <span className={`font-semibold ${hippsStyle?.textColor}`}>Revenue Impact:</span>
                  <span className={`ml-2 font-bold ${hippsStyle?.textColor}`}>
                    ${(suggestion.hippsPoints * 2.15).toFixed(0)}
                  </span>
                </div>
                <div>
                  <span className={`font-semibold ${hippsStyle?.textColor}`}>Category:</span>
                  <span className={`ml-2 font-bold ${hippsStyle?.textColor}`}>
                    {suggestion.hippsPoints >= 20 ? 'Critical' : 
                     suggestion.hippsPoints >= 15 ? 'High Value' :
                     suggestion.hippsPoints >= 10 ? 'Medium Value' :
                     suggestion.hippsPoints >= 5 ? 'Standard' : 'Minimal'}
                  </span>
                </div>
              </div>
            </div>
          ...
          {/* AI Reasoning */}
          <div>
            <h4 className="font-bold text-gray-800 mb-2 text-sm">AI Reasoning</h4>
          ...
          {/* Supporting Evidence */}
          <div>
            <h4 className="font-bold text-gray-800 mb-2 text-sm">Supporting Evidence</h4>
            ...
            {hasExistingComments && (
              <div className="space-y-2 mb-3">
                {existingComments.map((comment) => {
                  ...
                  return (
                    <div key={comment.id} className="bg-gray-50 p-3 rounded border transition-all duration-200">
                      {!isEditingThisComment ? (
                        <>
                          <p className="text-sm text-gray-700 mb-2 font-medium">{comment.text}</p>
                          <div className="flex items-center justify-between">
                            <span className="text-xs text-gray-500 font-medium">{comment.timestamp}</span>
                            <button
                              onClick={() => onStartEditingComment(suggestion.id, comment.id)}
                        ...
                        <div className="space-y-2">
                          <textarea
                            ...
                            onChange={(e) => {
                              ...
                              onCommentChange(commentKey, e.target.value, cursorPosition);
                            }}
                          ...
                          <div className="flex gap-2">
                            <button
                              onClick={() => onSubmitEditComment(suggestion.id, comment.id)}
                            ...
                            <button
                              onClick={() => onCancelEditComment(suggestion.id, comment.id)}
            ...
            {isAddingNewComment && (
              <div className="space-y-3">
                <textarea
                  ...
                  onChange={(e) => {
                    const cursorPosition = e.target.selectionStart;
                    onCommentChange(`new-${suggestion.id}`, e.target.value, cursorPosition);
                  }}
