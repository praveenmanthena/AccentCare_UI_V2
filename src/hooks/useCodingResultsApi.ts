...
export const useCodingResultsApi = (docId: string | null) => {
  ...
  const transformSupportingInfo = (
    supportingInfo: ApiSupportingInfo[],
    codeId: string
  ): SupportingSentence[] => {
    return supportingInfo.map((info, index) => ({
      ...
      id: `${codeId}-${index}`,
      boundingBox:
        info.bbox && info.bbox.length > 0
          ? convertBboxToBoundingBox(info.bbox.flat())
          : undefined,
    }));
  };
  ...
  ): { hippsPoints: number; isHippsContributor: boolean } => {
  ...
  const transformApiCodeSuggestion = (
    apiCode: ApiCodeSuggestion,
    isPrimary: boolean
  ): CodeSuggestion => {
    // Use API code_id if available, otherwise generate one
    const codeId =
      apiCode.code_id ||
      `${apiCode.diagnosis_code}-${apiCode.rank}-${
        isPrimary ? "primary" : "secondary"
      }`;

    // Get HIPPS data for this specific ICD code
    const { hippsPoints, isHippsContributor } = getHippsData(
      apiCode.diagnosis_code
    );
    ...
    return {
      ...
      supportingSentences: transformSupportingInfo(
        apiCode.supporting_info,
        codeId
      ),
      addedTimestamp: apiCode.created_at
        ? new Date(apiCode.created_at).toLocaleString()
        : undefined,
      ...
      supportingSentenceForActiveDisease:
        apiCode.supporting_sentence_for_active_disease,
      activeManagementAsOfJune2025: apiCode.active_management_asof_1june2025,
      supportingSentenceForActiveManagement:
        apiCode.supporting_sentence_for_active_management,
  ...
  const fetchCodingResults = async (docId: string) => {
    try {
      ...
      const data: CodingResultsApiResponse = await apiClient.get(
        `/coding-results/${docId}`
      );
      ...
      data.results.primary_codes.forEach((code) => {
        if (code.comments && code.comments.length > 0) {
          const codeId =
            code.code_id || `${code.diagnosis_code}-${code.rank}-primary`;
      ...
      data.results.secondary_codes.forEach((code) => {
        if (code.comments && code.comments.length > 0) {
          const codeId =
            code.code_id || `${code.diagnosis_code}-${code.rank}-secondary`;
          allComments[codeId] = transformApiComments(code.comments);
        }
      });
      ...
      setError(
        err instanceof Error ? err.message : "Failed to fetch coding results"
      );
